...
- **Type:** `string | string[]`
- **Default:** `'modules'`
- **Related:** [Browser Compatibility](/cn/guide/build#browser-compatibility)
...
Note: the polyfill does **not** apply to [Library Mode](/cn/guide/build#library-mode). If you need to support browsers without native dynamic import, you should probably avoid using it in your library.
...
Specify the output directory (relative to [project root](/cn/guide/#index-html-and-project-root)).
...
Specify the directory to nest generated assets under (relative to `build.outDir`. This is not used in [Library Mode](/cn/guide/build#library-mode)).
...
- **Related:** [Dynamic Import](/cn/guide/features#dynamic-import)
...
- **Related:** [Library Mode](/cn/guide/build#library-mode)
...
- **Type:** `boolean | string`
- **Default:** `false`
- **Related:** [Backend Integration](/cn/guide/backend-integration)
...
- **Type:** `boolean | string`
- **Default:** `false`
- **Related:** [Server-Side Rendering](/cn/guide/ssr)
...
Set to `false` to disable writing the bundle to disk. This is mostly used in [programmatic `build()` calls](/cn/guide/api-javascript#build) where further post processing of the bundle is needed before writing to disk.
