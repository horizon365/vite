...
1. In your Vite config, configure the entry and enable build manifest:
   ...
   If you haven't disabled the [module preload polyfill](/cn/config/build-options.md#build-polyfillmodulepreload), you also need to import the polyfill in your entry
...
2. For development, inject the following in your server's HTML template (substitute `http://localhost:5173` with the local URL Vite is running at):
   ...
   - Set [`server.origin`](/cn/config/server-options.md#server-origin) so that generated asset URLs will be resolved using the back-end server URL instead of a relative path
...
3. For production: after running `vite build`, a `.vite/manifest.json` file will be generated alongside other asset files. An example manifest file looks like this:
   ...
   - For the CSS file generated when [`build.cssCodeSplit`](/cn/config/build-options.md#build-csscodesplit) is `false`, the key is `style.css`.
