...
Note that Vite will also resolve [its config file (i.e. `vite.config.js`)](/es/config/#configuring-vite) inside the project root, so you'll need to move it if the root is changed.
