...
Note that if you are using non-relative [`base`](/ko/config/shared-options.md#base), you must prefix each key with that `base`.
...
In some cases, you might also want to configure the underlying dev server (e.g. to add custom middlewares to the internal [connect](https://github.com/senchalabs/connect) app). In order to do that, you need to write your own [plugin](/ko/guide/using-plugins.html) and use [configureServer](/ko/guide/api-plugin.html#configureserver) function.
...
- **Related:** [Warm Up Frequently Used Files](/ko/guide/performance.html#warm-up-frequently-used-files)
...
- **Related:** [appType](./shared-options#apptype), [SSR - Setting Up the Dev Server](/ko/guide/ssr#setting-up-the-dev-server)
...
Vite will search for the root of the potential workspace and use it as default. A valid workspace met the following conditions, otherwise will fall back to the [project root](/ko/guide/#index-html-and-project-root).
...
Accepts a path to specify the custom workspace root. Could be a absolute path or a path relative to [project root](/ko/guide/#index-html-and-project-root). For example:
