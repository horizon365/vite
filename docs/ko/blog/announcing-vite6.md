...
Today, we're taking another big step in Vite's story. The Vite [team](/ko/team), [contributors](https://github.com/vitejs/vite/graphs/contributors), and ecosystem partners are excited to announce the release of Vite 6.
...
If you're new to Vite, we suggest reading the [Getting Started](/ko/guide/) and [Features](/ko/guide/features) guides first.
...
- [Default value for `resolve.conditions`](/ko/guide/migration#default-value-for-resolve-conditions)
- [JSON stringify](/ko/guide/migration#json-stringify)
- [Extended support of asset references in HTML elements](/ko/guide/migration#extended-support-of-asset-references-in-html-elements)
- [postcss-load-config](/ko/guide/migration#postcss-load-config)
- [Sass now uses modern API by default](/ko/guide/migration#sass-now-uses-modern-api-by-default)
- [Customize CSS output file name in library mode](/ko/guide/migration#customize-css-output-file-name-in-library-mode)
- [And more changes that should only affect a few users](/ko/guide/migration#advanced)

There is also a new [Breaking Changes](/ko/changes/) page that lists all planned, considering, and past changes in Vite.

## Migrating to Vite 6

For most projects, the update to Vite 6 should be straightforward, but we advise reviewing the [detailed Migration Guide](/ko/guide/migration) before upgrading.
