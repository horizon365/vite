...
::: tip NOTE
When using [middleware mode](/ko/config/server-options.html#server-middlewaremode) combined with [proxy config for WebSocket](/ko/config/server-options.html#server-proxy), the parent http server should be provided in `middlewareMode` to bind the proxy correctly.
...
**Related:** [server.fs.allow](/ko/config/server-options.md#server-fs-allow)
...
Pre-processes `.css`, `.scss`, `.sass`, `.less`, `.styl` and `.stylus` files to plain CSS so it can be used in browsers or parsed by other tools. Similar to the [built-in CSS pre-processing support](/ko/guide/features#css-pre-processors), the corresponding pre-processor must be installed if used.
